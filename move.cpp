...
/***************************************************
 * MOVE : CONSTRUCTOR WITH POSSIBLE MOVES
 * Initialize a move from source and destination positions,
 * finding the matching move in the set of possible moves
 * to get the correct move type and capture information
 ***************************************************/
Move::Move(const Position& src, const Position& dest, const set<Move>& possibleMoves)
{
   // Set the basic source and destination
   source = src;
   this->dest = dest;
   
   // Find matching move in possible moves to get the correct move type and capture info
   for (const Move& possibleMove : possibleMoves)
   {
      if (possibleMove.source == src && possibleMove.dest == dest)
      {
         // Copy all properties from the matching move
         *this = possibleMove;
         return;
      }
   }
   
   // If no match found, default to basic move
   moveType = MOVE;
   capture = SPACE;
   promote = SPACE;
   enpassant = false;
   castleK = false;
   castleQ = false;
   isWhite = false;  // Will be set later if needed
   text = "";
   error = "";
}
